		...
		for(int c=0;c<len;c++) {
			...
			if(
				...
				if(encoded==null) {
					...
					if(c>0) encoded.append(value, 0, c);
				}
				if(ch=='\\') encoded.append("\\\\");
				else if(ch=='\b') encoded.append("\\b");
				else if(ch=='\f') encoded.append("\\f");
				else if(ch=='\t') encoded.append("\\t");
				else {
					int ich=ch;
					encoded
						.append("\\u")
				...
				if(encoded!=null) encoded.append(ch);
			}
		}
		...
		for(int c=0;c<len;c++) {
			...
			if(ch=='\\') {
				...
				if(++c<len) {
					ch=encoded.charAt(c);
					if(ch=='\\') value.append('\\');
					else if(ch=='b' || ch=='B') value.append('\b');
					else if(ch=='f' || ch=='F') value.append('\f');
					else if(ch=='t' || ch=='T') value.append('\t');
					else if(ch=='u' || ch=='U') {
						if(++c<len) {
									...
									if(
										...
										if(++c<len) {
											...
											if(
												...
												} else value.append('\\').append(ch).append(ch1).append(ch2).append(ch3);
											} else value.append('\\').append(ch).append(ch1).append(ch2).append(ch3);
										} else value.append('\\').append(ch).append(ch1).append(ch2);
									} else value.append('\\').append(ch).append(ch1).append(ch2);
								} else value.append('\\').append(ch).append(ch1);
							} else value.append('\\').append(ch).append(ch1);
						} else value.append('\\').append(ch);
					} else value.append('\\').append(ch);
				} else value.append('\\');
			} else {
				if(value!=null) value.append(ch);
			}
		}
