...
public class RandomAccessFileBuffer extends AbstractPersistentBuffer {
    ...
    public int read(long position, byte[] buff, int off, int len) throws IOException {
        raf.seek(position);
    ...
    public void write(long position, byte[] buff, int off, int len) throws IOException {
        if((position+len)>length()) throw new EOFException();
        raf.seek(position);
