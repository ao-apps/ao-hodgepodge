...
final public class IntegerRadixSortNew extends SortAlgorithm<Number> {
	...
	private static final int LARGE_THRESHOLD = 1048576; // TODO: Tune: 65536;
	...
	private static final ThreadLocal<int[][]> fromQueues = !USE_SMALL_THREAD_LOCAL ? null : new ThreadLocal<int[][]>() {
		@Override
		protected int[][] initialValue() {
			return new int[PASS_SIZE][LARGE_THRESHOLD];
		}
	};
	private static final ThreadLocal<int[][]> toQueues = !USE_SMALL_THREAD_LOCAL ? null : new ThreadLocal<int[][]>() {
		@Override
		protected int[][] initialValue() {
			return new int[PASS_SIZE][LARGE_THRESHOLD];
		}
	...
	private static <T extends Number> void sortSmall(List<T> list, SortStatistics stats) {
		...
		int[][] fromQueues = (USE_SMALL_THREAD_LOCAL ? IntegerRadixSortNew.fromQueues.get() : new int[PASS_SIZE][size]);
		int[] fromQueueLenths = new int[PASS_SIZE];
		@SuppressWarnings("unchecked")
		int[][] toQueues = (USE_SMALL_THREAD_LOCAL ? IntegerRadixSortNew.toQueues.get() : new int[PASS_SIZE][size]);
